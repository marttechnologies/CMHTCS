from account.db_models.mini_models import IDTracker 
...
def recursive_id_gen(
    ...
    """
    Generate User ID Based on sequence of letters and numbers recursively.
    It follows that when the next ID is requested, the function retrieves the last ID from IDTracker model
    and increments it by one. Then it calculates the new ID based on the last ID.
    
    AA0000 -> AA0001 -> AA0002 -> AA0003 ...
    
    For staff, it makes the number of numbers 2 making it a 4-digit ID by default 
    For guardian, it makes the number of numbers 3 making it a 5-digit ID by default 
